...
def cae(_X, _W, _b, _keepprob):
    ...
    return {'input_r': _input_r, 'ce1': _ce1, 'ce2': _ce2, 'ce3': _ce3, 'ce4': _ce4,
        'layers': (_input_r, _ce1, _ce2, _ce3, _ce4)
        , 'out': _out}
print ("Network ready")
...
with tf.name_scope("encoded_reshape"):
   encoded_reshape = tf.reshape(cae(x, weights, biases, 1)['out'], shape=[-1, 4*4*3*n5])
...
with tf.Session() as sess:
    ...
    if RETRAIN:
        ...
        for epoch_i in range(n_epochs):
            ...
            if (epoch_i % 400) == 0:
                save_path = saver.save(sess, MODEL_FILE_DIR + "cae_vd_mch_elu_full_"+ timestr +"_"+ str(epoch_i) +".ckpt")
    ...
    if CLASSIFY:
        nn_batch_size = int(batch_size / 3)+1
        nn_n_epochs   = 20000
