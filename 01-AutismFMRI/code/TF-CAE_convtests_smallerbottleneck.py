...
with tf.name_scope('learning_rate') as scope:
    ...
    learning_rate =  tf.train.exponential_decay(0.00004, global_step, 800, 0.98)
...
with tf.Session() as sess:
    ...
    train_writer = tf.train.SummaryWriter('./tensorflow_logs/train_64bn_norm_'+time+'/', sess.graph)
    test_writer = tf.train.SummaryWriter('./tensorflow_logs/test_64bn_norm_'+time+'/', sess.graph)
    ...
    for epoch_i in range(n_epochs):
        for batch_i in range(fmri.train.num_examples // batch_size):
            ...
            batch_mean = np.mean(batch_xs, 0)
            trainbatch = np.array([img - batch_mean for img in batch_xs])
