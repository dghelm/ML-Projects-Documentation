...
def read_data_sets(datafile,
    ...
    sub = f["subjects"]
    ids = [x for x in sub]
    dims = (slice(0,60),slice(8,68),slice(0,60))
    ...
    dots = [np.zeros([60,60,60]) for x in range(channels)]
    ...
    def sub_imgs(ID):
        ...
        # dot_img = np.pad(dot_img, ( (5,5),(0,0),(6,6) ), 'constant' )

        axisx_img = np.amax(dot_img, axis=2)
        axisx_img = np.ravel(axisx_img)
        axisy_img = np.amax(dot_img, axis=1)
        axisy_img = np.ravel(axisy_img)
        axisz_img = np.amax(dot_img, axis=0)
        axisz_img = np.ravel(axisz_img)


        sub_img = axisx_img
        sub_img = np.vstack( (sub_img, axisy_img))
        sub_img = np.vstack( (sub_img, axisz_img))
