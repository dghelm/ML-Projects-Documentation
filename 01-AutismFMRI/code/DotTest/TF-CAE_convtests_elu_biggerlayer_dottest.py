...
MODEL_FILE = "cae_elu_big_dt_161116-140309_699.ckpt"
LOAD_FEATURES = False
CLASSIFY = True
...
def cae(_X, _W, _b, _keepprob):
    ...
    with tf.name_scope('encoders') as scope:
        ...
        _ce1 = tf.nn.elu(tf.add(tf.nn.conv3d(_input_r, _W['ce1']
            , strides=[1, 1, 1, 1, 1], padding='SAME'), _b['be1']), )
...
with tf.name_scope("encoded_reshape"):
   encoded_reshape = tf.reshape(cae(x, weights, biases, 1)['ce4'], shape=[-1, 7*8*6*n4])

nn_keep_prob = tf.placeholder(tf.float32)

hidden1 = nn_layer(encoded_reshape, (7*8*6*n4), (4*3*3*n2),'layer1')
...
with tf.Session() as sess:
    ...
    train_writer = tf.train.SummaryWriter('../tensorflow_logs/train_elu_dt_big_'+timestr+'/', sess.graph)
    test_writer = tf.train.SummaryWriter('../tensorflow_logs/test_elu_dt_big_'+timestr+'/', sess.graph)
    init = tf.initialize_all_variables()
    sess.run(init)

    # batch_size = 16
    batch_size = (int(len(testlabels)/4))+1
    n_epochs   = 700
    epoch_i = 0
    ...
    if RETRAIN:
        ...
        for epoch_i in range(n_epochs):
            ...
            if (epoch_i % 500) == 0:
                save_path = saver.save(sess, MODEL_FILE_DIR + "cae_elu_dt_big_"+ timestr +"_"+ str(epoch_i) +".ckpt")
    ...
    if CLASSIFY:
        nn_batch_size = int(batch_size/2)+1
        nn_n_epochs   = 2000
