...
MODEL_FILE = "cae_elu_dt_161116-140309_699.ckpt"
LOAD_FEATURES = True
CLASSIFY = True
...
with tf.Session() as sess:
    ...
    train_writer = tf.train.SummaryWriter('../tensorflow_logs/train_elu_dottest_'+timestr+'/', sess.graph)
    test_writer = tf.train.SummaryWriter('../tensorflow_logs/test_elu_dottest_'+timestr+'/', sess.graph)
    init = tf.initialize_all_variables()
    sess.run(init)
    ...
    if RETRAIN:
        ...
        for epoch_i in range(n_epochs):
            ...
            if (epoch_i % 500) == 0:
                save_path = saver.save(sess, MODEL_FILE_DIR + "cae_elu_dt_"+ timestr +"_"+ str(epoch_i) +".ckpt")
