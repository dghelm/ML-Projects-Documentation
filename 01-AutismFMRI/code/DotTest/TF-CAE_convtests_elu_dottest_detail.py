...
MODEL_FILE = "cae_elu_dtd_161210-211128_699.ckpt"
LOAD_FEATURES = True
CLASSIFY = True
...
with tf.name_scope('learning_rate') as scope:
    learning_rate =  tf.train.exponential_decay(0.00001, global_step, 4000, 0.85)
...
with tf.Session() as sess:
    ...
    train_writer = tf.train.SummaryWriter('../tensorflow_logs/train_elu_dottest3d_'+timestr+'/', sess.graph)
    test_writer = tf.train.SummaryWriter('../tensorflow_logs/test_elu_dottest3d_'+timestr+'/', sess.graph)
    init = tf.initialize_all_variables()
    sess.run(init)

    # batch_size = 16
    batch_size = len(testlabels)/2
    n_epochs   = 700
    epoch_i = 0
    ...
    if RETRAIN:
        ...
        for epoch_i in range(n_epochs):
            ...
            if (epoch_i < 10) or (epoch_i % 150 == 0):
                test_xss, _ = fmri.test.next_batch(batch_size)
                test_xs = np.reshape(test_xss[0], [1, -1] )
                summary = sess.run(img_merged, feed_dict={x:test_xs, keepprob: 1.})
                test_writer.add_summary(summary, epoch_i)
                test_xs = np.reshape(test_xss[1], [1, -1] )
                summary = sess.run(img_merged, feed_dict={x:test_xs, keepprob: 1.})
                train_writer.add_summary(summary, epoch_i)

            if (epoch_i % 500) == 0:
                save_path = saver.save(sess, MODEL_FILE_DIR + "cae_elu_dt3d_"+ timestr +"_"+ str(epoch_i) +".ckpt")
    ...
    if CLASSIFY:
        ...
        for epoch_i in range(nn_n_epochs):
            ...
            if (epoch_i % 500) == 0:
                save_path = saver.save(sess, MODEL_FILE_DIR + "cae_dot3d_nn_"+ timestr +"_"+ str(epoch_i) +".ckpt")
