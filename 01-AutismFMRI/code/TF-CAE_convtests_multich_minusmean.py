...
with tf.Session() as sess:
    ...
    train_writer = tf.train.SummaryWriter('./tensorflow_logs/train_32-256_mch_elu_'+timestr+'/', sess.graph)
    test_writer = tf.train.SummaryWriter('./tensorflow_logs/test_32-256_mch_elu_'+timestr+'/', sess.graph)
    init = tf.initialize_all_variables()
    sess.run(init)
    ...
    if RETRAIN:
        ...
        for epoch_i in range(n_epochs):
            ...
            if (epoch_i % 500) == 0:
                save_path = saver.save(sess, MODEL_FILE_DIR + "cae_mch_elu_"+ timestr +"_"+ str(epoch_i) +".ckpt")
